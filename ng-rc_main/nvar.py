...
def get_linear(i, var, total_i):
    for j in range(i, i-k*s-1, -s): 
        total_i.append(var[j])
    return total_i

def get_total_i(total_i):
    ## now for nonlinear ##
...
for i in i_ls:
    total_i =[] 
    for var in soln_train:
        total_i=get_linear(i, var, total_i)
    total_vec_train.append(get_total_i(total_i)) # concatenate


Y_d = np.array(list(zip(x,y,z)))
if np.shape(total_vec_train)[0] < np.shape(Y_d)[0]:
    diff = np.shape(Y_d)[0] - np.shape(total_vec_train)[0]
    Yd = Y_d[:-diff, :]

## ridge regression to solve for minimizing W_out##
alpha = 1e-3
Yd_train = Y_d[:split_index]
...
def compute_nmse(Y_actual, Y_pred):
    return np.sqrt((Y_actual - Y_pred)**2 / len(Y_actual))

# apply nvar on training data
def nvar_validate(Y_actual):
    Y_pred = []
    i = s*k # initially at min value
    for Yi in Y_actual:
        print(np.shape(Yi)) # 3, so good
        total_i = []
        total_i = get_linear(i, Yi, total_i)
        print(np.shape(total_i)) # only 2!
        print(i)
        total_i = get_total_i(total_i)
        print(np.shape(total_i)) # syaing it's 5
        Yi_next = Yi + (W_out@total_i.T).T# simple Euler like step
...
def nvar_test(Y_in, limit):
    ...
    while len(Y_in) <= limit: # do for as long as we don't go over the limit
        # compute O total
        total_i = []
        total_i = get_linear(i, Y_in[-1], total_i)
        total_i = get_total_i(total_i)
        Yi_next = Y_in[-1] + (W_out@total_i.T).T # use the previous term to calculate the next
        Y_in = np.append(Y_in, Yi_next)
        i+=1
    return Y_in


# first compute over training points and compute the normalized root mean square error
Y_train = np.array(list(zip(x,y,z)))[:split_index]
Y_train_pred, nmse = nvar_validate(Y_train)
